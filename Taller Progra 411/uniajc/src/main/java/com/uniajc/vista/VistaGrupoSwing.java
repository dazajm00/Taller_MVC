...
/**
 * Vista Swing para la gestión de {@link Grupo}.
 * Usa JComboBox para seleccionar materia y docente.
 */
public class VistaGrupoSwing extends JPanel {
    ...
    private static final String[] COLUMNAS = {"ID", "Materia", "Docente", "Aula", "Horario", "Creado"};

    private final ControladorGrupo controlador;
    ...
    private JComboBox<String> cmbMateria, cmbDocente;
    private JTextField txtAula, txtHorario;
    ...
    public VistaGrupoSwing() {
        this.controlador = new ControladorGrupo(
            new GrupoService(new GrupoDao(), materiaDao, docenteDao));
        initUI();
        cargarTabla();
    }
    ...
    private JPanel panelBotones() {
        ...
        btnEditar.addActionListener(e -> {
            if (tabla.getSelectedRow() < 0)
                JOptionPane.showMessageDialog(this, "Seleccione un grupo.", "Aviso", JOptionPane.WARNING_MESSAGE);
    ...
    private void cargarTabla() {
        ...
        try {
            for (Grupo g : controlador.listarGrupos()) {
                modeloTabla.addRow(new Object[]{
                    g.getId(), g.getNombreMateria(), g.getNombreDocente(),
                    g.getAula(), g.getHorario(),
                    g.getFechaCreacion() != null ? g.getFechaCreacion().toLocalDate() : ""
                });
            }
        } catch (Exception ex) {
    ...
    private void filtrarTabla() {
        ...
        try {
            for (Grupo g : controlador.listarGrupos()) {
                if ((g.getNombreMateria() != null && g.getNombreMateria().toLowerCase().contains(busq))
                 || (g.getNombreDocente() != null && g.getNombreDocente().toLowerCase().contains(busq))
                 || g.getAula().toLowerCase().contains(busq)
                 || g.getHorario().toLowerCase().contains(busq)
                 || String.valueOf(g.getId()).contains(busq)) {
                    modeloTabla.addRow(new Object[]{
                        g.getId(), g.getNombreMateria(), g.getNombreDocente(),
                        g.getAula(), g.getHorario(),
                        g.getFechaCreacion() != null ? g.getFechaCreacion().toLocalDate() : ""
    ...
    private void abrirDialogo(boolean edicion) {
        ...
        if (edicion) {
            ...
            controlador.buscarGrupo(id).ifPresent(g -> grupoSeleccionado = g);
        }

        // Cargar listas para los combos
        listaMaterias = materiaDao.obtenerTodos();
        listaDocentes = docenteDao.obtenerTodos();

        dialogo = new JDialog((JFrame) SwingUtilities.getWindowAncestor(this),
                edicion ? "Editar Grupo" : "Nuevo Grupo", true);
        dialogo.setSize(450, 340);
        ...
        JLabel lTitulo = new JLabel(edicion ? " Editar Grupo" : " Nuevo Grupo", SwingConstants.CENTER);
        ...
        // Combo Materias
        gbc.gridy = 1; gbc.gridx = 0; form.add(etiqueta("Materia:"), gbc);
        cmbMateria = new JComboBox<>();
        cmbMateria.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        for (Materia m : listaMaterias) cmbMateria.addItem(m.getId() + " – " + m.getNombre());
        if (edicion && grupoSeleccionado.getIdMateria() > 0) {
            for (int i = 0; i < listaMaterias.size(); i++) {
                if (listaMaterias.get(i).getId() == grupoSeleccionado.getIdMateria()) {
                    cmbMateria.setSelectedIndex(i); break;
                }
            }
        }
        gbc.gridx = 1; form.add(cmbMateria, gbc);

        // Combo Docentes
        gbc.gridy = 2; gbc.gridx = 0; form.add(etiqueta("Docente:"), gbc);
        cmbDocente = new JComboBox<>();
        cmbDocente.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        for (Docente d : listaDocentes) cmbDocente.addItem(d.getId() + " – " + d.getNombre());
        if (edicion && grupoSeleccionado.getIdDocente() > 0) {
            for (int i = 0; i < listaDocentes.size(); i++) {
                if (listaDocentes.get(i).getId() == grupoSeleccionado.getIdDocente()) {
                    cmbDocente.setSelectedIndex(i); break;
                }
            }
        }
        gbc.gridx = 1; form.add(cmbDocente, gbc);

        gbc.gridy = 3; gbc.gridx = 0; form.add(etiqueta("Aula:"), gbc);
        txtAula = campo(grupoSeleccionado.getAula());
        gbc.gridx = 1; form.add(txtAula, gbc);

        gbc.gridy = 4; gbc.gridx = 0; form.add(etiqueta("Horario:"), gbc);
        txtHorario = campo(grupoSeleccionado.getHorario());
        gbc.gridx = 1; form.add(txtHorario, gbc);
    ...
    private void guardar() {
        if (listaMaterias.isEmpty() || listaDocentes.isEmpty()) {
            JOptionPane.showMessageDialog(dialogo, "Debe haber materias y docentes registrados primero.", "Aviso", JOptionPane.WARNING_MESSAGE);
            return;
        }
        int idxMateria = cmbMateria.getSelectedIndex();
        int idxDocente = cmbDocente.getSelectedIndex();
        if (idxMateria < 0 || idxDocente < 0) {
            JOptionPane.showMessageDialog(dialogo, "Seleccione materia y docente.", "Validación", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int idMateria = listaMaterias.get(idxMateria).getId();
        int idDocente = listaDocentes.get(idxDocente).getId();

        String resultado = modoEdicion
            ? controlador.actualizarGrupo(grupoSeleccionado.getId(), idMateria, idDocente,
                txtAula.getText(), txtHorario.getText())
            : controlador.registrarGrupo(idMateria, idDocente,
                txtAula.getText(), txtHorario.getText());
    ...
    private void eliminar() {
        ...
        if (JOptionPane.showConfirmDialog(this, "¿Eliminar el grupo seleccionado?",
                "Confirmar", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            int id = (int) modeloTabla.getValueAt(tabla.getSelectedRow(), 0);
            String r = controlador.eliminarGrupo(id);
